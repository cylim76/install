...
blocked_hosts() {
	echo
	while :; do
		echo -e "是否开启广告拦截(会影响性能)(AD block) [${magenta}Y/N$none]"
		read -p "$(echo -e "(默认(Default) [${cyan}N$none]):")" blocked_ad
		[[ -z $blocked_ad ]] && blocked_ad="n"

		case $blocked_ad in
		Y | y)
			blocked_ad_info="开启(On)"
			ban_ad=true
			echo
			echo
			echo -e "$yellow 广告拦截(AD block) = $cyan开启(On)$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
		N | n)
			blocked_ad_info="关闭(Off)"
			echo
			echo
			echo -e "$yellow 广告拦截(AD block) = $cyan 关闭(Off) $none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
	...
	while :; do
		...
		[1-9] | [1-9][0-9] | [1-9][0-9][0-9] | [1-9][0-9][0-9][0-9] | [1-5][0-9][0-9][0-9][0-9] | 6[0-4][0-9][0-9][0-9] | 65[0-4][0-9][0-9] | 655[0-3][0-5])
			...
			if [[ $tls && $ssport == "80" ]] || [[ $tls && $ssport == "443" ]]; then
				...
				echo
				echo
				echo -e "$yellow Shadowsocks 端口(Port) = $cyan$ssport$none"
				echo "----------------------------------------------------------------"
				echo
				break
			fi
...
shadowsocks_password_config() {
	...
	while :; do
		echo -e "请输入 "$yellow"Shadowsocks"$none" 密码(Passwd)"
		read -p "$(echo -e "(默认密码(default): ${cyan}233blog.com$none)"): " sspass
		[ -z "$sspass" ] && sspass="233blog.com"
		case $sspass in
		*[/$]*)
			echo
			echo -e " 由于这个脚本太辣鸡了..所以密码不能包含(must not includes)$red / $none或$red $ $none这两个符号.... "
			echo
			error
			;;
		*)
			echo
			echo
			echo -e "$yellow Shadowsocks 密码(Passwd) = $cyan$sspass$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
shadowsocks_ciphers_config() {
	...
	while :; do
		echo -e "请选择 "$yellow"Shadowsocks"$none" 加密协议(choose encrypt protocal) [${magenta}1-${#ciphers[*]}$none]"
		...
		echo
		read -p "$(echo -e "(默认加密协议(default encrypt protocal): ${cyan}${ciphers[1]}$none)"):" ssciphers_opt
		...
		[1-3])
			...
			echo
			echo
			echo -e "$yellow Shadowsocks 加密协议(encrypt protocal) = $cyan${ssciphers}$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
install_info() {
	...
	echo
	echo "---------- 安装信息(installed info) -------------"
	echo
	echo -e "$yellow V2Ray 传输协议(trans protocal) = $cyan${transport[$v2ray_transport - 1]}$none"

	if [[ $v2ray_transport == [45] || $v2ray_transport == 33 ]]; then
		echo
		echo -e "$yellow V2Ray 端口(Port) = $cyan$v2ray_port$none"
		echo
		echo -e "$yellow 你的域名(your domain name) = $cyan$domain$none"
		echo
		echo -e "$yellow 域名解析(your domain solves to) = ${cyan}我确定已经有解析了$none"
		echo
		echo -e "$yellow 自动配置(Automatically install) TLS = $cyan$install_caddy_info$none"

		if [[ $ban_ad ]]; then
			echo
			echo -e "$yellow 广告拦截(AD block) = $cyan$blocked_ad_info$none"
		fi
		if [[ $is_path ]]; then
			echo
		...
		echo -e "$yellow V2Ray 端口(Port) = $cyan$v2ray_port$none"
		echo
		echo -e "$yellow V2Ray 动态端口范围(Dynamic ports range) = $cyan${v2ray_dynamic_port_start_input} - ${v2ray_dynamic_port_end_input}$none"
	...
	if [ $shadowsocks ]; then
		...
		echo
		echo -e "$yellow 是否配置(whether installed) Shadowsocks = ${cyan}未配置(No)${none}"
	fi
	echo
	echo "---------- END -------------"
	echo
...
domain_check() {
	...
	if [[ $test_domain != $ip ]]; then
		echo
		echo -e "$red 检测域名解析错误(domain name resolving error)....$none"
		echo
		echo -e " 你的域名: $yellow$domain$none 未解析到(your domain name doesn't resolve to): $cyan$ip$none"
		echo
		echo -e " 你的域名当前解析到(your domain name resolves to): $cyan$test_domain$none"
		echo
...
install_v2ray() {
	...
	if [[ ! -d /etc/v2ray/233boy/v2ray ]]; then
		...
		echo
		echo -e " 温馨提示..... 请尝试自行安装 Git: ${green}$cmd install -y git $none 之后再安装此脚本(Please install Git manually)"
		echo
		exit 1
	fi
...
pause() {
	...
	read -rsp "$(echo -e "按 $green Enter 回车键(Enter) $none 继续(to continue)....或按 $red Ctrl + C $none 取消(to Cancel).")" -d $'\n'
	echo
}
do_service() {
	if [[ $systemd ]]; then
...
install() {
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		echo
		echo " 大佬...你已经安装 V2Ray 啦...无需重新安装(You've already installed V2RAY"
		echo
		echo -e " $yellow输入 ${cyan}v2ray${none} $yellow即可管理(to manage) V2Ray${none}"
		echo
		exit 1
	elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/233blog_v2ray_backup.txt && -d /etc/v2ray/233boy/v2ray ]]; then
		echo
		echo "  如果你需要继续安装.. 请先卸载旧版本(Uninstall old V2RAY first"
		echo
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		...
		if [[ $mark ]]; then
			...
			echo
			echo -e " $yellow输入 ${cyan}v2ray uninstall${none} $yellow即可卸载(uninstall)${none}"
			echo
		fi

	elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/233blog_v2ray_backup.txt && -d /etc/v2ray/233boy/v2ray ]]; then
		echo
		echo -e " $yellow输入 ${cyan}v2ray uninstall${none} $yellow即可卸载(to uninstall)${none}"
		echo
	else
		echo -e "
		$red 大胸弟...你貌似毛有安装 V2Ray ....卸载个鸡鸡哦(There is no V2RAY to UNINSTALL)...$none

		备注...仅支持卸载使用我 (233v2.com) 提供的 V2Ray 一键安装脚本(Only can be used to uninstall V2RAY installed with my scripts)
		" && exit 1
	fi
...
*)
	echo
	echo -e " 你输入的这个参数 <$red $args $none> ...这个是什么鬼啊...脚本不认识它哇(Wrong parameter)"
	echo
	echo -e " 这个辣鸡脚本仅支持输入$green local / online $none参数"
	echo
	echo -e " 输入$yellow local $none即是使用本地安装(Local install)"
	echo
	echo -e " 输入$yellow online $none即是使用在线安装(Online install) (默认Default)"
	echo
	exit 1
	;;
esac
...
while :; do
	...
	echo
	echo "帮助说明(Help): https://233v2.com/post/1/"
	echo
	echo "搭建教程(How to construct): https://233v2.com/post/2/"
	echo
	echo " 1. 安装(Install)"
	echo
	echo " 2. 卸载(Uninstall)"
	echo
	if [[ $local_install ]]; then
		echo -e "$yellow 温馨提示.. 本地安装(Local install)已启用 ..$none"
		echo
	fi
	read -p "$(echo -e "请选择(please choose) [${magenta}1-2$none]:")" choose
	case $choose in
	1)
		install
		break
